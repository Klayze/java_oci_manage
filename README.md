...
| **MCP 接入** | Claude Code、Codex、Cursor 等 AI agent 经面板操作授权主机，SSH 凭据不下发给 agent |
...
- **终端贴图** — 快捷键 / 右键 / 拖放粘图，图片传到远端后粘入路径，供 Claude Code、Codex 附图
...
- **DNS 管理** — Cloudflare 域名记录的增删改查（支持 API Token 最小权限认证）
- **云监控** — 按账号设流量阈值超额自动关停、实例停机通知与自动拉起
- **域名监控** — 每日检查域名与 SSL 证书到期，分档 Telegram 提醒，可从 Cloudflare 一键导入
- **多账号隔离** — 按 Profile 单独配置 API 出站代理，Profile 可一键复制为新地区
- **客户端维护** — 浏览器里查版本、升级、重启客户端，实时查看运行日志
- **对象存储** — OCI Object Storage 的 Bucket 和文件管理
- **Email Delivery** — 一键搭建邮件域（DKIM/DNS/SMTP 全自动）、测试发信
- **AWS 管理** — EC2 实例创建/管理/删除与防火墙安全组管理，Lightsail 轻量实例创建/列表/电源/删除/流量/网络 IP 管理，费用统计
...
| [MCP 接入](md/mcp.md) | 让 AI agent 通过面板操作主机：签发令牌、配置客户端、安全须知 |
