...
档案里存了指纹的连接，校验发生在**认证之前**的密钥交换阶段：主机密钥对不上直接断开，密码和私钥根本不会发出去。没存指纹的连接行为不变。
...
往终端里粘图片。图片会传到远端的临时目录，终端里粘进去的是那张图在远端的绝对路径 —— Claude Code、Codex 这类工具按路径就能把图附上。

在本机终端里它们靠读系统剪贴板贴图；网页终端里它们跑在远端，读到的是远端的空剪贴板，终端通道本身又只传文本，所以走的是这条路径。
...
支持 PNG / JPG / GIF / WebP，单张不超过 20MB，多张按顺序粘并用空格分隔。

剪贴板里有文本时一律按文本粘贴，原有行为不受影响。远端临时文件超过 24 小时自动清理。
